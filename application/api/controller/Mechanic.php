...
class Mechanic extends Main
{
     ...
     * 获取技工列表
     */
    public function getMechanicLis(){
        if(request()->isPost()){
            ...
            if(isset($post['ages']) && $post['ages']){//技工经验
                $where['A.ages'] = $post['ages'];
            }
            if(isset($post['cates1']) && $post['cates1'] && !$post['cates2']){
                $option = $post['cates1'];
                $where[] = ['exp','FIND_IN_SET('.$option.',A.position)'];
            }
            if(isset($post['cates2']) && $post['cates2']){
                $option = $post['cates2'];
                $where[] = ['exp','FIND_IN_SET('.$option.',A.position)'];
            }
            $where['A.status']=['eq',0];
            ...
            if($data){
                foreach($data as &$v){
                    ...
                    $type = array_column($type,'title');
                    $v['position'] = implode(" ",$type);
    ...
    public function getMechanicInfoById(){
        if(request()->isPost()){
            ...
            if($data){
                ...
                $type = Db::name('member_attr')->where(['id'=>['in',$data['position']]])->field('title')->select();
                $type = array_column($type,'title');
                $data['position'] = implode(" ",$type);
     ...
     * 获取技工案例列表
     */
    public function getMechanicCasesLis(){
        if(request()->isPost()){
            $post = $this->request->post();
